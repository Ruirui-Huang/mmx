...
from mmx.models.layers import AttentionBlock
...
class SegmenterMaskTransformerHead(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        for i in range(num_layers):
            self.layers.append(
                AttentionBlock(embed_dims=embed_dims,
                               num_heads=num_heads,
        ...
        self.output_upscaling = nn.Sequential(
            nn.ConvTranspose2d(embed_dims, embed_dims // 4, kernel_size=2, stride=2),
            nn.BatchNorm2d(embed_dims // 4),
            nn.GELU(),
            nn.ConvTranspose2d(embed_dims // 4, self.channels, kernel_size=2, stride=2),
            # nn.GELU(),
        )
        ...
        self.patch_proj = nn.Linear(self.channels, self.channels, bias=False)
        self.classes_proj = nn.Linear(embed_dims, self.channels, bias=False)
    ...
    def forward(self, inputs):
        ...
        # embedding = patches.transpose(1, 2).view(b, -1, h, w)
        # upscaled_embedding = self.output_upscaling(embedding)
        # b, c, h, w = upscaled_embedding.shape
        # patches = upscaled_embedding.permute(0, 2, 3, 1).contiguous().view(b, -1, c)

        embedding = x[:, :-self.num_classes].transpose(1, 2).view(b, -1, h, w)
        upscaled_embedding = self.output_upscaling(embedding)
        b, c, h, w = upscaled_embedding.shape
        upscaled_embedding = upscaled_embedding.permute(0, 2, 3, 1).contiguous().view(b, -1, c)

        patches = self.patch_proj(upscaled_embedding)
