...
def parse_args():
    ...
    # args.config = "configs/det/vitdet_base_auto_sample.py"
    # args.work_dir = "runs/det/debug_0818_2"
...
def merge_args(cfg, args):
    ...
    # set dataloader args
    default_dataloader_cfg = ConfigDict(pin_memory=True,
    ...
    # 修改mlflow的run_name
    if args.work_dir is not None:
        run_name = os.path.basename(cfg.work_dir)
        if len(cfg["visualizer"]["vis_backends"]) > 1:
...
def main():
    ...
    # 打印当前运行的cv任务
    print_log(f"Currently running with {cfg.default_scope}", logger='current')
    # start training
    runner.train()
